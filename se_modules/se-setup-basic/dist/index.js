...
var seMenuBasic = require('se-menu-basic');
...
function insertImageItem(nodeType) {
  return new seMenuBasic.MenuItem({
    title: "Insert image",
    label: "Image",
...
function cmdItem(cmd, options) {
  ...
  return new seMenuBasic.MenuItem(passedOptions)
}
...
function markItem(markType, options) {
  var passedOptions = {
    ...
    select: prosemirrorCommands.toggleMark(markType) && ( function (state) { return canInsertMark(state, markType); } ), //if this is present blocktype will be removed if not applicable
    // enable: true
    enable: function enable(state) { 
...
function linkItem(markType) {
  return new seMenuBasic.MenuItem({
    title: "Ctrl+Shift+l",
    icon: seMenuBasic.icons.link,
    active: function active(state) { return markActive(state, markType) },
    select: prosemirrorCommands.toggleMark(markType) && ( function (state) { return ((!state.selection.empty && canInsertBlock(state, markType))  // selection
...
function buildMenuItems(schema) {
  ...
  if (type = schema.marks.strong)
    { r.toggleStrong = markItem(type, {title: "Cmd+b", icon: seMenuBasic.icons.strong}); }
  if (type = schema.marks.em)
    { r.toggleEm = markItem(type, {title: "Cmd+i", icon: seMenuBasic.icons.em}); }
  if (type = schema.marks.underline)
    { r.toggleUnderline = markItem(type, {title: "Cmd+u", icon: seMenuBasic.icons.underline}); }
  if (type = schema.marks.code)
    { r.toggleCode = markItem(type, {title: "Cmd+`", icon: seMenuBasic.icons.code}); }
  ...
  if (type = schema.nodes.bullet_list)
    { r.wrapBulletList = wrapListItem(type, {
      title: "Ctrl+Shift+8",
      icon: seMenuBasic.icons.bulletList
    }); }
  if (type = schema.nodes.ordered_list)
    { r.wrapOrderedList = wrapListItem(type, {
      title: "Ctrl+Shift+9",
      icon: seMenuBasic.icons.orderedList
    }); }
  if (type = schema.nodes.blockquote) {
    ...
    // })
    // CONVERT
    r.makeBlockQuote = seMenuBasic.blockTypeItem(type, {
      title: "Ctrl+Shift+>",
      icon: seMenuBasic.icons.blockquote
    }, schema.nodes.paragraph);
  }
  if (type = schema.nodes.paragraph)
    { r.makeParagraph = seMenuBasic.blockTypeItem(type, {
      title: "Ctrl+Shift+0",
      label: "Plain"
    }); }
  if (type = schema.nodes.code_block)
    { r.makeCodeBlock = seMenuBasic.blockTypeItem(type, {
      title: "Cmd+Shift+\\",
      label: "Code"
    }, schema.nodes.paragraph); }
  if (type = schema.nodes.heading)
    ...
    */
    { for (var i = 1; i <= schema.nodes.heading.spec.parseDOM.length; i++)
      { r["makeHead" + (i)] = seMenuBasic.blockTypeItem(type, {                         
        title: "Ctrl+Shift+" + (i),                                       
        // label: "Level " + (i),                                         
        icon: seMenuBasic.icons["heading"+ (i)],                                      
        attrs: {level: (i+1)}                                               //+1 if h1 is missing in the heading node spec
      }, schema.nodes.paragraph); } }
  if (type = schema.nodes.horizontal_rule) {
    var hr = type;
    r.insertHorizontalRule = new seMenuBasic.MenuItem({
  ...
  // NEW:
    ...
    r.blockMenu = [cut([r.wrapBulletList, r.wrapOrderedList, r.makeBlockQuote, seMenuBasic.joinUpItem,
                        seMenuBasic.liftItem])];
    ...
    // VALID:
    if (isIOS()) {
      r.fullMenu = r.inlineMenu.concat(r.typeMenu, [[seMenuBasic.undoItem, seMenuBasic.redoItem]], r.blockMenu);
    } else {
...
function blockQuoteRule(nodeType) {
  ...
  return prosemirrorInputrules.textblockTypeInputRule(/^\s*>\s$/, nodeType) // change to
}
...
function buildKeymap(schema, mapKeys) {
                  ...
                  if (state.selection.empty && node.childCount > 1 
                    ...
                    // CALL CMD
                      if (state.selection.$cursor.nodeAfter!=null) {
                        ...
                        var cmd_ = prosemirrorSchemaList.liftListItem(node.type);
                        ...
                        var cmd_$1 = prosemirrorSchemaList.liftListItem(node.type); //node.type); //schema.nodes.list_item)
  ...
  if (type = schema.nodes.blockquote) {
    ...
    bind("Ctrl->", prosemirrorCommands.setBlockType(type,null, schema.nodes.paragraph));  // change existing element to blockquote
  }
                  ...
                  // YOUTUBE find match
                    ...
                    if (video_matches!=null){
                      ...
                      // WT/ CUT
                        ...
                        // Get content after
                          var wrap_cut = prosemirrorModel.Fragment.from($from.node(0).cut($from.pos,$from.end())); 
                        // create new content node
                          var wrap_paste = prosemirrorModel.Fragment.empty; //state.schema.nodes.paragraph.createAndFill();
                        // Loop cuttted fragment
                          var addchildren = function (node, pos) {                          
                            if (pos>1) {
                              // console.log("\tnode",node,pos);
                              wrap_paste = wrap_paste.append(prosemirrorModel.Fragment.from(node));
                              return false;
                            }
                          };
                          wrap_cut.descendants(addchildren);
                        // Create fragment from new p node and and cutted content 
                          wrap_paste = prosemirrorModel.Fragment.from(state.schema.nodes.paragraph.create(null,wrap_paste));
                        // Create video fragment and 
                            ...
                            wrap_video = prosemirrorModel.Fragment.from(state.schema.nodes.figure.create(null, prosemirrorModel.Fragment.from(state.schema.nodes.div.create({style:attr_style,src:attr_src}))));
                        // Append cutted content  to fragment with video
                          wrap_video = wrap_video.append(wrap_paste);                        
                        // Dispatch
                          opt_tr_ = state.tr;
                          if ($from.pos==$from.end()) {
                            opt_tr_.replaceRange(pPos.from, pPos.to,new prosemirrorModel.Slice(wrap_video,0,0) );// 1,1 = to next block; 0,0 = to new empty block (has issues at the end of doc)
                          } else {
                            opt_tr_.replaceRange(pPos.from, pPos.to,new prosemirrorModel.Slice(wrap_video,1,1) );// 1,1 = to next block; 0,0 = to new empty block (has issues at the end of doc)
                          }        
          ...
          // special cases
            for (var d = $from.depth; d >= 0; d--) {
              ...
              if (node.type.name=="list_item" && $from.parent.content.size != 0) {
                  ...
                  var split_ = prosemirrorSchemaList.splitListItem(node.type, opt_tr_);
                ...
                // Move to figcaption or split figure
                  ...
                  if (state.selection.empty) {
                    ...
                    if (nodeAfterParent===null) {
                      ...
                      // Add paragraph
                      var last_node = prosemirrorModel.Fragment.from(state.schema.nodes.paragraph.create());
 ...
 // }
          ...
          for (var d = $from.depth; d >= 0; d--) {
            ...
            if (node.type.name=="list_item") {              
              ...
              if (range) {
                  ...
                  // }
                  var cmd_ = prosemirrorSchemaList.sinkListItem(node.type);
  ...
  if (type = schema.nodes.code_block)
    { bind("Shift-Ctrl-\\", prosemirrorCommands.setBlockType(type, null, schema.nodes.paragraph)); }
  if (type = schema.nodes.heading) {
    // console.info(" schema.nodes.heading ", schema.nodes.heading.spec.parseDOM.length);
    for (var i = 1; i <= schema.nodes.heading.spec.parseDOM.length; i++) {
      bind("Shift-Ctrl-" + i, prosemirrorCommands.setBlockType(type, {level: i+1}, schema.nodes.paragraph));  // +1 because only one h1 is allowed
    }
  }
  if (type = schema.nodes.horizontal_rule) {
    var hr = type;
      ...
      if (opt_cut_) {
        // CUT to NEW BLOCK
          ...
          // Add 
            ...
            if ($from.pos==$from.end()) {
              tr.replace(pPos.pos, pPos.pos,new prosemirrorModel.Slice(wrap_paste,0,0) );// 1,1 = to next block; 0,0 = to new empty block (has issues at the end of doc)
            } else {
              tr.replace(pPos.pos, pPos.pos,new prosemirrorModel.Slice(wrap_paste,1,1) );// 1,1 = to next block; 0,0 = to new empty block (has issues at the end of doc)
            }        
        ...
        // JUST ESCAPE TO NEW P BLOCK
          ...
          if ($to.pos==$to.end()) {
            ...
            var frag_ = prosemirrorModel.Fragment.from(state.schema.nodes.paragraph.create());        
            var tr$1 = opt_tr_ || state.tr;
            tr$1.replace(pPos.pos, pPos.pos, new prosemirrorModel.Slice(frag_, 0, 0) );// 1,1 = to next block; 0,0 = to new empty block (has issues at the end of doc)
 ...
 * @type {PluginKey}
 */
var extensionPluginKey = new prosemirrorState.PluginKey("extension");
...
function extensionPlugin(schema, menuPlugin, uploaderClass) {	
	 ...
	 */
	var pluginClass = new prosemirrorState.Plugin(
									{
													...
													if (action && action.add) {
														...
														// Upload placeholder - add DOM for upload placeholder (SVG)
															...
															// Create Widget Decoration
																var deco = prosemirrorView.Decoration.widget(action.add.pos, widget, {id: action.add.id, name: "imageUpload", stopEvent: function (evt) {
																	// stop all
																	return true;
																} } );
															// Add to set
														...
														// Upload placeholder - add DOM wrapper for upload placeholder
															...
															var deco$1 = prosemirrorView.Decoration.node(action.open.pos.from, action.open.pos.to, {class: 'simpleeditor-figure-build'},{id: action.open.id, name: "figwrap" });
														...
														// Upload placeholder - remove all
															...
															if (action.removeAll == null) {
																set = prosemirrorView.DecorationSet.empty;
															} else {
																set = set.remove(set.find(null, null, function (spec) { return (spec.name == action.removeAll.name &&  spec.id.uploader == action.removeAll.uploader ); }));
															}
														...
														// Add video source decoration (called from Block menu)
															var videoSource_id = {};
															var deco_videoSource = prosemirrorView.Decoration.node(action.videoSource.pos.from, action.videoSource.pos.to, {class: 'simpleeditor-embed-node'},{id: videoSource_id, name: "videoSource" });
														...
														if (action.emptyNode) {
															...
															if (Array.isArray(action.emptyNode)) {
																for (var i = 0; i < action.emptyNode.length; i++) {
																	...
																	deco_emptyNode = prosemirrorView.Decoration.node(action.emptyNode[i].pos.from, action.emptyNode[i].pos.to, {class: "simpleeditor-empty-node"},{id: emptyNode_id_, name: "emptyNode"});
																	deco_.push(deco_emptyNode);
																}															} else {
																emptyNode_id_ = new Date().getTime(); //action.emptyNode[i].id;
																deco_emptyNode = prosemirrorView.Decoration.node(action.emptyNode.pos.from, action.emptyNode.pos.to, {class: "simpleeditor-empty-node"},{id: emptyNode_id_, name: "emptyNode"});
																deco_.push(deco_emptyNode);
															}
														}
														...
														if (action.selectedEmptyNode) {
										                    ...
										                    var deco_selectedEmptyNode = prosemirrorView.Decoration.node(action.selectedEmptyNode.pos.from-1, action.selectedEmptyNode.pos.to-1, {class: 'simpleeditor-empty-node-selected'},{id: selectedEmptyNode_id, name: "selectedEmptyNode"});
															// Add to array
														...
														if (action.figureCaption) {
										                  		...
										                  		// Add to decoration
												                    ...
												                    var deco_figureCaption = prosemirrorView.Decoration.widget(action.figureCaption.pos.from, figcap_el, {key: figureCaption_key.toString(), name: "figureCaption" , side: -1, ignoreSelection: true, stopEvent: function (evt) {
										                        	...
										                        	// Create menu wrapper
										                  			...
										                  			// Create buttons
										                  				...
										                  				// IMG button
										                  					...
										                  					el_img.appendChild(seMenuBasic.getIcon(seMenuBasic.icons.image,"simpleeditor-menu-tooltip-icon"));
										                  				...
										                  				// Video button
										                  					...
										                  					el_vid.appendChild(seMenuBasic.getIcon(seMenuBasic.icons.video,"simpleeditor-menu-tooltip-icon"));										                  					
										                  				...
										                  				// HR button
										                  					...
										                  					el_hr.appendChild(seMenuBasic.getIcon(seMenuBasic.icons.hr,"simpleeditor-menu-tooltip-icon"));
										                  			...
										                  			// Add to decoration
													                    ...
													                    var deco_blockMenu = prosemirrorView.Decoration.widget(action.blockMenu.pos.from, menu_tt_el, {key: blockMenu_key.toString(), name: "blockMenu" , side: -1, ignoreSelection: true, stopEvent: function (evt) {
													                    	// stop all - https://prosemirror.net/docs/ref/#view.Decoration^widget^spec.stopEvent
													                    	return true;
													                    } });
			...
			*/
				...
				if (typeof oTransactions['insertCaption'] != "undefined") {
					...
					for (var i = 0; i < oTransactions['insertCaption'].length; i++) {
						// Add caption but cursor must be set in subsequesnt dispatch call
							wrap_cap = new prosemirrorModel.Slice(prosemirrorModel.Fragment.from(oState.schema.nodes.figcaption.create()),0,0);
			...
			// Add HR Node
				if (typeof oTransactions['replaceWithHr'] != "undefined") {
					...
					for (var i = 0; i < oTransactions['replaceWithHr'].length; i++) {
						...
						//NEW wt/ new p
							wrap_hr_ = prosemirrorModel.Fragment.empty;
							wrap_hr_ = wrap_hr_.append(prosemirrorModel.Fragment.from(oState.schema.nodes.horizontal_rule.create()));
							wrap_hr_ = wrap_hr_.append(prosemirrorModel.Fragment.from(oState.schema.nodes.paragraph.create()));						 	
							 wrap_hr_ = new prosemirrorModel.Slice(wrap_hr_,1,1);
			...
			// Add Text Selection
				if (typeof oTransactions['textSelection'] != "undefined") {
					...
					if (newPos!==null) {
						var newSel = new prosemirrorState.TextSelection.between(newPos,newPos);
						try {
		...
		// Vars
			...
			switch (action) {
				case "open":
					id = "figwrap_"+data;
					// Build wrapper
					wrap_img = prosemirrorModel.Fragment.empty;				
					wrap_img = wrap_img.append(prosemirrorModel.Fragment.from(oView.state.schema.nodes.figure.createAndFill()));
					wrap_img = wrap_img.append(prosemirrorModel.Fragment.from(oView.state.schema.nodes.paragraph.createAndFill()));
					wrap_img = new prosemirrorModel.Slice(wrap_img,0,0);						
...
// ============================================
	...
	pluginCoreClass.prototype.mUnlink = function mUnlink (oView, fn, oV)
	{
		...
		// NEW:
		prosemirrorCommands.toggleMark(oView.state.schema.marks.link)(oView.state, oView.dispatch);
          return true
...
// ============================================
	...
	pluginCoreClass.prototype._mSelect = function _mSelect (oView, oPos, opt_scroll_) {
		...
		// Create Text Selection
			...
			var sel = new prosemirrorState.TextSelection.between($s, $e);			
		// Focus editor
			oView.focus({
...
// ============================================
	...
	pluginCoreClass.prototype.initLinkDialog = function initLinkDialog ()
	{
		...
		this.linkDialog.c.appendChild(seMenuBasic.getIcon(seMenuBasic.icons.cancel,"simpleeditor-menu-tooltip-icon"));			
...
function setup(options, uploaderClass) {
  ...
  // Define plugings
    ...
    // Menu Bar plugin
    var menuPlugin = seMenuBasic.menuBar({floating: options.floatingMenu !== false, content: options.menuContent || buildMenuItems(options.schema).fullMenu});
